...
function CaseStream(lineStream, getNumOfLineForCaseByFirstLine) {
  ...
  if (typeof getNumOfLineForCaseByFirstLine === 'number') {
    ...
    buffered = withCount.buffer(function() {
...
function ValidatorStream(outputStream, caseStream, pathSpec) {
  ...
  //.combineLatest(caseStream, function(zipped, jamCase) {
  //  return {
  ...
  //return outputStream.filter(function(outputs) {
  //  return String(outputs.output) !== lines[outputs.caseNum - 1];
  //});
}
